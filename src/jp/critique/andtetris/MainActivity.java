...
public class MainActivity extends SimpleBaseGameActivity implements OnClickListener{
	...
	public void onCreateResources() {
		...
		this.backgroundBitmapTextureAtlas = new BitmapTextureAtlas(this.getTextureManager(), 300, 750, TextureOptions.BILINEAR);
		this.backgroundTextureRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(this.backgroundBitmapTextureAtlas, this, "bg.png", 0, 0);
		this.backgroundBitmapTextureAtlas.load();
		
		this.arrowRightBitmapTextureAtlas = new BitmapTextureAtlas(this.getTextureManager(), 120, 120, TextureOptions.BILINEAR);
		this.arrowRightTextureRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(this.arrowRightBitmapTextureAtlas, this, "arrow-right.png", 0, 0);
		this.arrowRightBitmapTextureAtlas.load();
		
		this.arrowLeftBitmapTextureAtlas = new BitmapTextureAtlas(this.getTextureManager(), 120, 120, TextureOptions.BILINEAR);
		this.arrowLeftTextureRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(this.arrowLeftBitmapTextureAtlas, this, "arrow-left.png", 0, 0);
		this.arrowLeftBitmapTextureAtlas.load();
		
		this.rotateBitmapTextureAtlas = new BitmapTextureAtlas(this.getTextureManager(), 120, 120, TextureOptions.BILINEAR);
		this.rotateTextureRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(this.rotateBitmapTextureAtlas, this, "rotate.png", 0, 0);
	...
	public Scene onCreateScene() {
		...
		final Sprite backgroundSprite = new Sprite(0, 0, backgroundTextureRegion, this.getVertexBufferObjectManager());
		...
		// create buttons
		final Sprite leftArrowSprite = new ButtonSprite(- (this.arrowLeftTextureRegion.getWidth() * 2) / 3, MainActivity.CAMERA_HEIGHT - this.arrowLeftTextureRegion.getWidth() / 3 - 20, this.arrowLeftTextureRegion, this.getVertexBufferObjectManager(),this);
		leftArrowSprite.setZIndex(Integer.MAX_VALUE);
		tetrisContainer.attachChild(leftArrowSprite);
		
		final Sprite rightArrowSprite = new ButtonSprite(MainActivity.CONTAINER_WIDTH - (this.arrowRightTextureRegion.getWidth() * 1) / 3, MainActivity.CAMERA_HEIGHT - this.arrowRightTextureRegion.getWidth() / 3 - 20, this.arrowRightTextureRegion, this.getVertexBufferObjectManager(),this);
		rightArrowSprite.setZIndex(Integer.MAX_VALUE);
		tetrisContainer.attachChild(rightArrowSprite);
		
		final Sprite rotateArrowSprite = new ButtonSprite(MainActivity.CONTAINER_WIDTH / 2 - this.rotateTextureRegion.getWidth() / 2, MainActivity.CAMERA_HEIGHT - (this.rotateTextureRegion.getWidth()) / 3 - 10, this.rotateTextureRegion, this.getVertexBufferObjectManager(),this);
		...
		tetrisContainer.registerUpdateHandler(new IUpdateHandler() {

			@Override
			public void onUpdate(float pSecondsElapsed) {
				
				if(MainActivity.this.block != null) {
					Entity block = MainActivity.this.block.getEntity();
...
//		}
	...
	private void moveBlock(final String direction) {
		runOnUiThread(new Runnable() {
			...
			@Override
			public void run() {
				final Entity block = MainActivity.this.block.getEntity();
//				block.clearEntityModifiers();
				...
				if(direction == "right")
					block.registerEntityModifier(new MoveModifier(0.3f, x, x + Board.TILE_SIZE, y, y));
	...
	public void onClick(final ButtonSprite pButtonSprite, float pTouchAreaLocalX,
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if(pButtonSprite.getX() > MainActivity.CONTAINER_WIDTH - MainActivity.this.arrowRightTextureRegion.getWidth()) {
